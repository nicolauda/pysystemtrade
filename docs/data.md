...
If you need to repair gaps for an instrument that already has data in the DB without overwriting existing overlapping values, use [`/sysinit/futures/adhoc/repair_from_ib_to_db_non_destructive.py`](/sysinit/futures/adhoc/repair_from_ib_to_db_non_destructive.py). The script asks for the instrument code when started, merges per-contract intraday/daily/mixed prices conservatively by timestamp (existing DB rows always win on overlap, including partial rows/NaN fields), and writes only timestamps that are missing in DB. After the repair it can optionally rebuild roll calendar CSV plus multiple/adjusted prices; this step is opt-in so you can keep existing calendars unchanged when you only need a local gap fill. Calendar rebuild is conservative: it refreshes only roll dates that can be regenerated from currently available data, leaves non-regenerated dates untouched, and adds any new dates that become available. Duplicate roll transitions (same current/next/carry contracts on different timestamps) are automatically deduplicated by keeping the earliest timestamp. During the follow-up multiple-price rebuild, the script trims any stale leading prefix only in memory, so unsupported old rows cannot break the stitch while the stored CSV roll calendar remains intact. Derived-series writes are tail-only: the rebuilt candidate replaces only the rebuildable recent window, while older existing multiple/adjusted history before that cutoff is preserved unchanged. Multiple prices are regenerated without re-adjusting roll dates again (`adjust_calendar_to_prices=False`) to avoid accidental truncation on sparse contract overlaps.
...
If you need to backfill older history for an instrument that already has data in the DB (for example, prepend 1990-2000 to an existing 2000+ history), use [`/sysinit/futures/adhoc/import_from_bc_to_db_non_destructive.py`](/sysinit/futures/adhoc/import_from_bc_to_db_non_destructive.py). Contract prices are merged conservatively by timestamp: gaps are added, coherent existing rows are preserved, and overlaps are replaced only when existing rows are incoherent and incoming rows are coherent. The importer auto-detects common Barchart final-price column names (`Latest`, `Close`, `Last`) and coalesces `FINAL` row-by-row across available columns (preserving non-null primary values and filling missing rows/values from fallbacks). After contract import, rebuild of roll calendar plus multiple/adjusted prices is explicit opt-in (`[y/N]`) to avoid accidental destructive-looking rebuilds. If enabled, roll calendar generation first tries a full-history build and falls back to recent-to-past when stale; calendar writing is conservative by available dates, safely handles instruments with no pre-existing roll-calendar CSV, and skips duplicate contract transitions on different timestamps. Rebuilt multiple and adjusted prices still use conservative merge, but now include a safety fallback: if merged multiple prices show backward `PRICE_CONTRACT` transitions, the script stores the freshly rebuilt candidate (and aligned adjusted prices) to keep contract progression monotonic.
The same row-wise `FINAL` coalescing logic is also used by [`/sysinit/futures/adhoc/import_from_bc_to_db.py`](/sysinit/futures/adhoc/import_from_bc_to_db.py) through [`/sysinit/futures/contract_prices_from_split_freq_csv_to_db.py`](/sysinit/futures/contract_prices_from_split_freq_csv_to_db.py).
