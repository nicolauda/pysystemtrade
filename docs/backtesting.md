...
- Output: reports/plots go to `backtest_results/` next to the config (QuantStats HTML, PDF summary, txt summary, terminal log; PNGs are kept only if `--keep-intermediate-figs`). PNG generation includes a single notional-position chart (`notional_positions_<timestamp>.png`, no capped-positions variant) plus a single combined-forecast chart with one line per instrument (`combined_forecasts_<timestamp>.png`, sourced from `combForecast.get_combined_forecast(...)`). Use `--no-report` to skip the txt summary and `--no-debug-log` to skip the `.log`. Very wide tables (e.g., correlation matrices) are automatically split across multiple PDF pages/column blocks to stay readable; long rule names are wrapped and the first column auto-expands to avoid truncation.
- Capital from DB: `--use-db-capital` pulls `notional_trading_capital` and `base_currency` from Mongo/Parquet using the strategy key (`--strategy-name`, default: config filename stem). Useful when reusing the same YAML with different DB capital entries; the run will fail if that strategy name has no capital in the DB.
...
- IC-only inspection: add `--ic-only` (usually together with `--cache`/`--cache-file`) to skip the full backtest run and only recompute rule-level information coefficients using a cached system; the runner writes `ic_report_<timestamp>.pdf`/`.txt` that contains the rule-variation and aggregated-rule tables with the IC column so you can sanity-check signals without regenerating every plot.
...
system.rawdata.methods.methods()  ## works for any stage or data
```
